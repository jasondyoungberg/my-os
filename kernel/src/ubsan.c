...
void __ubsan_handle_pointer_overflow(void* data_raw, void* pointer_raw) {}
...
ubsan_abort(const struct ubsan_source_location* location,
            const char* violation) {
    if (!location || !location->filename)
        location = &unknown_location;

    panic("UBSAN: %s @ %s:%u:%u", violation, location->filename, location->line,
          location->column);
}
...
struct ubsan_float_cast_overflow_data {
    // TODO: Remove this GCC 5.x compatibility after switching to GCC 6.x.
    // The
    //       GCC developers accidentally forgot the source location. Their
    //       libubsan probes to see if it looks like a path, but we don't
    //       need to maintain compatibility with multiple gcc releases. See
    //       below.
